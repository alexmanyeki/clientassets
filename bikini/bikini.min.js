var ix=Webflow.require('ix');$(document).ready(function(){$('.map-modal').css({'pointer-events':'none'});$('.close-map-icon').click(function(){$('.map-modal').css({'pointer-events':'none'})});$('#mapTriggerMenu,#mapTriggerFooter').click(function(){$('.map-modal').css({'pointer-events':'all'})});function initialize(){var gmarkers=[];var map=null;var infowindow=null;var color='#e51772';var styles=[{"featureType":"administrative","elementType":"labels.text.fill","stylers":[{"color":"#444444"}]},{"featureType":"landscape","elementType":"geometry.fill","stylers":[{"color":"#e51772"}]},{"featureType":"poi","elementType":"all","stylers":[{"visibility":"simplified"}]},{"featureType":"poi","elementType":"geometry.fill","stylers":[{"color":"#e51772"},{"lightness":"42"}]},{"featureType":"poi","elementType":"labels.text","stylers":[{"saturation":"0"},{"lightness":"-100"},{"gamma":"0.00"},{"color":"#1a1718"}]},{"featureType":"poi","elementType":"labels.text.fill","stylers":[{"color":"#000000"}]},{"featureType":"poi","elementType":"labels.icon","stylers":[{"saturation":"-100"},{"gamma":"0.2"},{"lightness":"-21"},{"weight":"0.01"}]},{"featureType":"road","elementType":"all","stylers":[{"saturation":"-100"},{"lightness":"-15"}]},{"featureType":"road","elementType":"geometry.fill","stylers":[{"saturation":"0"},{"lightness":"0"},{"color":"#e51772"},{"gamma":"1.20"}]},{"featureType":"road","elementType":"labels.text","stylers":[{"lightness":"-29"},{"weight":"0.01"}]},{"featureType":"road.highway","elementType":"all","stylers":[{"visibility":"simplified"}]},{"featureType":"road.arterial","elementType":"labels.icon","stylers":[{"visibility":"off"}]},{"featureType":"transit","elementType":"all","stylers":[{"visibility":"off"},{"saturation":"-100"}]},{"featureType":"water","elementType":"all","stylers":[{"color":"#ae155c"},{"visibility":"on"},{"lightness":"-52"}]}];var mapOptions={center:new google.maps.LatLng(-8.682853,115.157498),zoom:19,mapTypeId:google.maps.MapTypeId.ROADMAP,mapTypeControl:!1,zoomControl:!0,styles:styles};map=new google.maps.Map(document.getElementById("mainMap"),mapOptions);google.maps.event.addListener(map,'click',function(){infowindow.close()});var locations=[['Bikini Restaurant Bali',-8.682853,115.157503]];function setMarkers(locations){for(var i=0;i<locations.length;i++){var iconBase='https://daks2k3a4ib2z.cloudfront.net/595059ab1d62971873de8c32/';var location=locations[i];var myLatLng=new google.maps.LatLng(location[1],location[2]);var marker=new google.maps.Marker({position:myLatLng,animation:google.maps.Animation.DROP,map:map,icon:iconBase+'5954cd80ca71cb26ede78186_map-pin-icon.png',title:location[0]});google.maps.event.addListener(marker,"click",function(){map.setCenter(marker.getPosition());infowindow.setContent(this.html);infowindow.open(map,this)});gmarkers.push(marker);bounds=new google.maps.LatLngBounds(new google.maps.LatLng(-168.999999,-358.999999),new google.maps.LatLng(168.999999,358.999999));rect=new google.maps.Rectangle({bounds:bounds,fillColor:color,fillOpacity:0.5,strokeWeight:0,map:map})}}
setMarkers(locations)}
google.maps.event.addDomListener(window,'load',initialize);$('html, body').scrollTop(0);$(window).on('load',function(){setTimeout(function(){$('html, body').scrollTop(0)},0)});$(".section-link").hover(function(){$(this).children('.responsive-image-dark').css({'filter':'brightness(100%)','-webkit-filter':'brightness(100%)','-moz-filter':'brightness(100%)','-o-filter':'brightness(100%)','-ms-filter':'brightness(100%)'})},function(){$(this).children('.responsive-image-dark').css({'filter':'brightness(65%)','-webkit-filter':'brightness(65%)','-moz-filter':'brightness(65%)','-o-filter':'brightness(65%)','-ms-filter':'brightness(65%)'})});setTimeout(function(){$(window).scroll(function(){event.preventDefault();var $target1=$('.hero-preloader');var $target2=$('.preloader');var trigger1={"type":"click","preserve3d":!0,"stepsA":[{"transition":"transform 500ms ease 0","x":"0px","y":"-100%","z":"0px"},{"wait":"1ms"}],"stepsB":[]};var trigger2={"type":"click","stepsA":[{"wait":"450ms"},{"display":"none"}],"stepsB":[]};ix.run(trigger1,$target1);ix.run(trigger2,$target2);return!0})},2000);$('[data-close="all"]').click(function(){$(this).closest('.form-select,.form-select-times').triggerHandler('w-close')});$('#groupSize,#preferredPeriod,#preferredTime,#preferredDate,#preferredDateLoaded').attr('tabindex','-1');$('.form-field').attr("oninvalid","invalidInputs(this);");$('#phone').attr('type','tel');$('[data-select="groupsize"]').click(function(e){$('#groupSize').val($(this).text())});$('[data-select="period"]').click(function(e){$('#preferredPeriod').val($(this).text());$('#preferredTime').val('')});$('[data-select="time"]').click(function(e){$('#preferredTime').val($(this).text())});$('#preferredDate').change(function(){$(this).closest('.form-select').triggerHandler('w-close')});$("#preferredDate").change(function(){var m_names=new Array("January","February","March","April","May","June","July","August","September","October","November","December");var oldDate=new Date($(this).val());var d=oldDate.getDate();var m=oldDate.getMonth();var y=oldDate.getFullYear();$("#preferredDateLoaded").val(d+" "+m_names[m]+" "+y)})});var input=$('.form-field');function invalidInputs(input){if(input.value==''){event.preventDefault();input.setCustomValidity($('.form-error-message').text('*Review highlighted field(s)'));input.setCustomValidity('');$(input).addClass('formerror');$('.form-error-message').css('opacity','1')}
else if(input.validity.typeMismatch){event.preventDefault();input.setCustomValidity($(input).val(''));$('#email').setCustomValidity($(input).attr('placeholder','Please enter a valid email'));$('#phone').setCustomValidity($(input).attr('placeholder','Please enter a valid phone number'));input.setCustomValidity($('.form-error-message').text('*Review highlighted field(s)'));input.setCustomValidity('');$(input).addClass('formerror');$(input).css('color','#e51772');$('.form-error-message').css('opacity','1')}
else{event.preventDefault();input.setCustomValidity($('.form-error-message').text(''));input.setCustomValidity('');$('.form-error-message').css('opacity','0');$(input).removeClass('formerror');$(input).css('color','inherit')}
return!0}
$(function(){$('#groupBookingForm').submit(function(){$('#preferredDate',this).prop('disabled',!0);return!0})});var Webflow=Webflow||[];Webflow.push(function(){$('#groupBookingForm').submit(function(){event.preventDefault();$('.numInput,.flatpickr-input').remove();var $target1=$('.form-success');var $target2=$('.form-sending');var $target3=$('.form-success-title');var $target4=$('.form-success-response');var $target5=$('.form-success-link');var trigger1={"type":"tabs","stepsA":[{"display":"flex"},{"wait":"1ms"},{"opacity":1,"transition":"opacity 500ms ease 0"}],"stepsB":[]};var trigger2={"type":"tabs","stepsA":[{"wait":"1800ms"},{"opacity":0,"transition":"opacity 500ms ease 0"}],"stepsB":[]};var trigger3={"type":"tabs","stepsA":[{"wait":"2300ms"},{"opacity":1,"transition":"transform 700ms ease 0, opacity 500ms ease 0","x":"0px","y":"0px","z":"0px"}],"stepsB":[]};var trigger4={"type":"tabs","stepsA":[{"wait":"2600ms"},{"opacity":1,"transition":"transform 700ms ease 0, opacity 500ms ease 0","x":"0px","y":"0px","z":"0px"}],"stepsB":[]};var trigger5={"type":"tabs","stepsA":[{"wait":"2900ms"},{"opacity":1,"transition":"transform 700ms ease 0, opacity 500ms ease 0","x":"0px","y":"0px","z":"0px"}],"stepsB":[]};ix.run(trigger1,$target1);ix.run(trigger2,$target2);ix.run(trigger3,$target3);ix.run(trigger4,$target4);ix.run(trigger5,$target5);return!0})});var _extends=Object.assign||function(target){for(var i=1;i<arguments.length;i++){var source=arguments[i];for(var key in source){if(Object.prototype.hasOwnProperty.call(source,key)){target[key]=source[key]}}}return target};var _typeof=typeof Symbol==="function"&&typeof Symbol.iterator==="symbol"?function(obj){return typeof obj}:function(obj){return obj&&typeof Symbol==="function"&&obj.constructor===Symbol&&obj!==Symbol.prototype?"symbol":typeof obj};function FlatpickrInstance(element,config){var self=this;self._={};self._.afterDayAnim=afterDayAnim;self._bind=bind;self._compareDates=compareDates;self._setHoursFromDate=setHoursFromDate;self.changeMonth=changeMonth;self.changeYear=changeYear;self.clear=clear;self.close=close;self._createElement=createElement;self.destroy=destroy;self.isEnabled=isEnabled;self.jumpToDate=jumpToDate;self.open=open;self.redraw=redraw;self.set=set;self.setDate=setDate;self.toggle=toggle;function init(){self.element=self.input=element;self.instanceConfig=config||{};self.parseDate=FlatpickrInstance.prototype.parseDate.bind(self);self.formatDate=FlatpickrInstance.prototype.formatDate.bind(self);setupFormats();parseConfig();setupLocale();setupInputs();setupDates();setupHelperFunctions();self.isOpen=!1;self.isMobile=!self.config.disableMobile&&!self.config.inline&&self.config.mode==="single"&&!self.config.disable.length&&!self.config.enable.length&&!self.config.weekNumbers&&/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);if(!self.isMobile)build();bindEvents();if(self.selectedDates.length||self.config.noCalendar){if(self.config.enableTime){setHoursFromDate(self.config.noCalendar?self.latestSelectedDateObj||self.config.minDate:null)}
updateValue()}
